...
__declspec(dllexport) void* create_decoder(int* error)
{
	void* mpg;
	int	ret;
...
__declspec(dllexport) int feed_mpeg_header(void* mpg, const byte* data, long bufsize, long streamsize, wavinfo_t* sc)
{
...
__declspec(dllexport) int feed_mpeg_stream(void* mpg, const byte* data, long bufsize, byte* outbuf, size_t* outsize)
{
...
__declspec(dllexport) int open_mpeg_stream(void* mpg, void* file, pfread f_read, pfseek f_seek, wavinfo_t* sc)
{
	mpg123_handle_t* mh = (mpg123_handle_t*)mpg;
	int		ret, no;
...
__declspec(dllexport) int read_mpeg_stream(void* mpg, byte* outbuf, size_t* outsize)
{
...
__declspec(dllexport) int get_stream_pos(void* mpg)
{
	return mpg123_tell(mpg);
}

__declspec(dllexport) int set_stream_pos(void* mpg, int curpos)
{
	return mpg123_seek(mpg, curpos, SEEK_SET);
}

__declspec(dllexport) void close_decoder(void* mpg)
{
	mpg123_delete(mpg);
	mpg123_exit();
}
